...
Ở mức cơ bản nhất, bạn có thể dùng R như một máy tính đơn giản. Hãy xem những phép tính dưới đây:
...
Với kiến thức trên, bạn hãy dựa vào hướng dẫn dưới đây để hoàn thành bài tập.
...
- Gõ: `my_apples` dưới câu bình luận thứ hai để hiển thị ra giá trị của `my_apples`.
...
test_output_contains("my_apples", incorrect_msg = "Bạn đã yêu cầu R hiển thị biến `my_apples` ra console chưa?")
...
test_output_contains("my_apples + my_oranges",
                     incorrect_msg = "Hãy nhớ hiển thị ra kết quả của phép tính cộng giữa `my_apples` và `my_oranges`. Câu cốt mẫu trong phần mô tả thực ra đã làm lộ kết quả của phép tính này!")
...
test_output_contains("my_fruit", incorrect_msg = "Đừng bỏ dòng lệnh dùng để hiển thị giá trị của `my_fruit`.")
...
- Các giá trị luận lý (`TRUE` hoặc `FALSE`)được gọi là **giá trị lôgic** (logical)
